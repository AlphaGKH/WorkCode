...
c_int osqp_solve(OSQPWorkspace *work, MatCT *Xin) {
  c_int exitflag;
  c_int iter;
...
# endif // PRINTING
  ...
  MatCreate(Xin, work->data->n,1);
  int f;
  for ( f = 0; f < work->data->n; f++)
  {
	  //c_print("x%d = %f  ",f, work->solution->x[f]);
	  Xin->element[f][0] = work->solution->x[f];
  }
...
c_int osqp_cleanup(OSQPWorkspace *work) {
  ...
  if (work) { // If workspace has been allocated
    ...
    if (work->data) {
	  ...
	  if (work->data->q) c_free(work->data->q); work->data->q = NULL;

	  if (work->data->l) c_free(work->data->l); work->data->l = NULL;

	  if (work->data->u) c_free(work->data->u); work->data->u = NULL;
	  c_free(work->data); work->data = NULL;
    }

    // Free scaling
    if (work->settings->scaling) {
		if (work->scaling->D) c_free(work->scaling->D); work->scaling->D = NULL;

      if (work->scaling->Dinv) c_free(work->scaling->Dinv); work->scaling->Dinv = NULL;

      if (work->scaling->E) c_free(work->scaling->E); work->scaling->E = NULL;

      if (work->scaling->Einv) c_free(work->scaling->Einv); work->scaling->Einv = NULL;
      c_free(work->scaling); work->scaling = NULL;

      // Free workspace variables
      if (work->D_temp) c_free(work->D_temp); work->D_temp = NULL;

      if (work->D_temp_A) c_free(work->D_temp_A); work->D_temp_A = NULL;

      if (work->E_temp) c_free(work->E_temp); work->E_temp = NULL;
    }
    ...
    if (work->pol) {
      if (work->pol->Alow_to_A) c_free(work->pol->Alow_to_A); work->pol->Alow_to_A = NULL;

      if (work->pol->Aupp_to_A) c_free(work->pol->Aupp_to_A); work->pol->Aupp_to_A = NULL;

      if (work->pol->A_to_Alow) c_free(work->pol->A_to_Alow); work->pol->A_to_Alow = NULL;

      if (work->pol->A_to_Aupp) c_free(work->pol->A_to_Aupp); work->pol->A_to_Aupp = NULL;

      if (work->pol->x) c_free(work->pol->x); work->pol->x = NULL;

      if (work->pol->z) c_free(work->pol->z); work->pol->z = NULL;

      if (work->pol->y) c_free(work->pol->y); work->pol->y = NULL;
      c_free(work->pol); work->pol = NULL;
    }

    // Free other Variables
    if (work->constr_type) c_free(work->constr_type); work->constr_type = NULL;

    if (work->rho_vec) c_free(work->rho_vec); work->rho_vec = NULL;

    if (work->rho_inv_vec) c_free(work->rho_inv_vec); work->rho_inv_vec = NULL;

    if (work->x) c_free(work->x); work->x = NULL;

    if (work->z) c_free(work->z); work->z = NULL;

    if (work->xz_tilde) c_free(work->xz_tilde); work->xz_tilde = NULL;

    if (work->x_prev) c_free(work->x_prev); work->x_prev = NULL;

    if (work->z_prev) c_free(work->z_prev); work->z_prev = NULL;

    if (work->y) c_free(work->y); work->y = NULL;

    if (work->Ax) c_free(work->Ax); work->Ax = NULL;

    if (work->Px) c_free(work->Px); work->Px = NULL;

    if (work->Aty) c_free(work->Aty); work->Aty = NULL;

    if (work->delta_y) c_free(work->delta_y); work->delta_y = NULL;

    if (work->Atdelta_y) c_free(work->Atdelta_y); work->Atdelta_y = NULL;

    if (work->delta_x) c_free(work->delta_x); work->delta_x = NULL;

    if (work->Pdelta_x) c_free(work->Pdelta_x); work->Pdelta_x = NULL;

    if (work->Adelta_x) c_free(work->Adelta_x); work->Adelta_x = NULL;

    // Free Settings
    if (work->settings) c_free(work->settings); work->settings = NULL;

    // Free solution
    if (work->solution) {
		if (work->solution->x) c_free(work->solution->x); work->solution->x = NULL;

      if (work->solution->y) c_free(work->solution->y); work->solution->y = NULL;
      c_free(work->solution); work->solution = NULL;
    }

    // Free information
    if (work->info) c_free(work->info); work->info = NULL;

    // Free timer
# ifdef PROFILING

    if (work->timer) c_free(work->timer); work->timer = NULL;
# endif /* ifdef PROFILING */

    // Free work
    c_free(work); work = NULL;
  }
