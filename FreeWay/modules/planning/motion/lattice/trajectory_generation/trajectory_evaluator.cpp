...
#include "modules/planning/motion/lattice/trajectory_generation/piecewise_braking_trajectory_generator.h"
...
using dharma::common::SpeedPoint;
  ...
  for (const auto &lon_trajectory : lon_trajectories) {
    ...
    for (const auto &lat_trajectory : lat_trajectories) {
      ...
      cost_queue_.emplace(Trajectory1dPair(lon_trajectory, lat_trajectory),
                          cost);
      //      std::cout << cost_queue_.size() << std::endl;
    }
  }
...
double
TrajectoryEvaluator::Evaluate(const PlanningTarget &planning_target,
...
double
TrajectoryEvaluator::LatOffsetCost(const PtrTrajectory1d &lat_trajectory,
