...
using dharma::common::math::Box2d;
using dharma::common::math::lerp;
using dharma::common::math::PathMatcher;
using dharma::common::math::Polygon2d;
...
SLBoundary PathTimeGraph::ComputeObstacleBoundary(const std::vector<common::math::Vec2d>& vertices,
...
void PathTimeGraph::SetStaticObstacle(const Obstacle* obstacle,
    ...
    path_time_obstacle_map_[obstacle_id].set_bottom_left_point(
                MakePathTimePoint(sl_boundary.start_s(), 0.0));
    path_time_obstacle_map_[obstacle_id].set_bottom_right_point(
                MakePathTimePoint(sl_boundary.start_s(), FLAGS_trajectory_time_length));
    path_time_obstacle_map_[obstacle_id].set_upper_left_point(
                MakePathTimePoint(sl_boundary.end_s(), 0.0));
    path_time_obstacle_map_[obstacle_id].set_upper_right_point(
                MakePathTimePoint(sl_boundary.end_s(), FLAGS_trajectory_time_length));
    ...
    while (relative_time < time_range_.second) {
        ...
        if (path_time_obstacle_map_.find(obstacle->Id()) ==
            ...
            path_time_obstacle_map_[obstacle->Id()].set_bottom_left_point(
                        MakePathTimePoint(sl_boundary.start_s(), relative_time));
            path_time_obstacle_map_[obstacle->Id()].set_upper_left_point(
                        MakePathTimePoint(sl_boundary.end_s(), relative_time));
        }
...
STPoint PathTimeGraph::MakePathTimePoint(const double s, const double t) const {
...
std::vector<std::pair<double, double>> PathTimeGraph::GetPathBlockingIntervals(const double t) const {
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
    ...
    double ego_width =  common::VehicleParamHandle::GetParam().width();
